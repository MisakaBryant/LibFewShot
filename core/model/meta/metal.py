...
class METAL(MetaModel):
    ...
    def __init__(self, inner_param, feat_dim, **kwargs):
        ...
        self.loss_func = MetaLossNetwork(
            support_meta_loss_num_dim, inner_param)

        self.query_loss_func = MetaLossNetwork(query_num_dim, inner_param)

        self.loss_adapter = LossAdapter(
            support_adapter_num_dim, args=inner_param, num_loss_net_layers=2)

        self.query_loss_adapter = LossAdapter(
            query_num_dim, args=inner_param, num_loss_net_layers=2)
    ...
    def set_forward(self, batch):
        ...
        for i in range(episode_size):
            ...
            """
            # 都是x
            episode_support_image = support_image[i].contiguous(
            ).reshape(-1, c, h, w)
            episode_query_image = query_image[i].contiguous(
            ).reshape(-1, c, h, w)
            # 都是y
    ...
    def set_forward_adaptation(self, support_set, query_set, support_target, query_target):
        ...
        for i in range(
            ...
            episode_query_targets = query_target[i].reshape(-1)
            """
            tmp_preds = self.forward_output(
                x=torch.cat((support_set, query_set), 0))
            support_preds = tmp_preds[:-query_set.size(0)]
            query_preds = tmp_preds[-query_set.size(0):]
            # name_param of classifier
            weights = dict(self.classifier.named_parameters())
            # name_param of loss_func
            meta_loss_weights = dict(self.loss_func.named_parameters())
            # name_param of loss_query_func
            meta_query_loss_weights = dict(
                self.query_loss_func.named_parameters())

            support_task_state = []

            support_loss = F.cross_entropy(
                input=support_preds, target=support_target)
            ...
            updated_meta_loss_weights = self.loss_adapter(
                adapt_support_task_state, i, meta_loss_weights)

            support_y = torch.zeros(support_preds.shape).to(
                support_preds.device)
            support_y[torch.arange(support_y.size(0)), support_target] = 1
            support_task_state = torch.cat((
                support_task_state.view(
                    1, -1).expand(support_preds.size(0), -1),
                support_preds,
                support_y
            ), -1)
            ...
            instance_entropy = torch.sum(
                torch.exp(out_prob) * out_prob, dim=-1)
            query_task_state = torch.stack(query_task_state)
            query_task_state = torch.cat((
                query_task_state.view(
                    1, -1).expand(instance_entropy.size(0), -1),
                query_preds,
                instance_entropy.view(-1, 1)
            ), -1)

            query_task_state = (
                query_task_state - query_task_state.mean()) / (query_task_state.std() + 1e-12)
...
class MetaLossNetwork(nn.Module):
    ...
    def build_network(self):
        ...
        for i in range(self.num_steps):
            self.layer_dict['step{}'.format(i)] = MetaStepLossNetwork(
                self.input_dim, args=self.args)
    ...
    def forward(self, x, num_step, params=None):
        ...
        out = self.layer_dict['step{}'.format(num_step)](
            out, param_dict['step{}'.format(num_step)])
...
class StepLossAdapter(nn.Module):
    def __init__(self, input_dim, num_loss_net_layers, args):
        ...
        self.args = args
        # 2 for weight and bias, another 2 for multiplier and offset
    ...
    def forward(self, task_state, num_step, loss_params):
        ...
        generated_multiplier, generated_offset = torch.chunk(
            out, chunks=2, dim=-1)
...
class LossAdapter(nn.Module):
    def __init__(self, input_dim, num_loss_net_layers, args):
        ...
        for i in range(self.num_steps):
            self.loss_adapter.append(StepLossAdapter(
                input_dim, num_loss_net_layers, args))
